...
test(preset_one.name + ' w/o prefix', async () => {
...
test(preset_one.name + ' w/ prefix', async () => {
	if (uno_config.presets) {
		uno_config.presets[1] = preset_one();
	}
	const unocss_generator = createGenerator(uno_config);
	const { css } = await unocss_generator.generate(
		fixtures.map((v) => `one-${v}`),
	);
	const formatted_css = await format(css, {
		parser: 'css',
		useTabs: true,
	...
	expect(formatted_css).toMatchFileSnapshot(
		import.meta.filename + '.prefix.css',
	);
});
